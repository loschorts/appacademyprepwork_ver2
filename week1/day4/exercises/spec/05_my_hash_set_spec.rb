...
describe MyHashSet do
  ...
  describe "set interaction methods" do
    ...
    describe "#symmetric_difference" do
      it "returns both sets' unique values" do
        sym_dif = set1.symmetric_difference(set2)
        union = set1.union(set2)
        intersect = set1.intersect(set2)
        minus = union.minus(intersect)
        expect(sym_dif.to_a).to eq(minus.to_a)
      end
    end

    describe "#==(object)" do
      it "finds clones to be equal" do
        new_set = set1
        expect(new_set==set1).to be_truthy
      end
    end
