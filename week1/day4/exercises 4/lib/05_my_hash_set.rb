...
class MyHashSet
  ...
  def intersect(set2)
    MyHashSet.new(@store.select {|k,v| set2.include?(k)})
  end
...
#   elements contained in either `set1` or `set2`, but not both!
  def symmetric_difference(set2) 
    my_union = self.union(set2)
    my_int = self.intersect(set2)
    my_union.minus(my_int)
  end
...
#   `object` is a member of `set1`.
  def ==(object)
    return false if object.class != MyHashSet
    return false unless object.store.length == self.store.length
    return false unless self.symmetric_difference(object).store.empty?
    true
  end
