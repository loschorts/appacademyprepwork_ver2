...
class String
  def caesar(shift)
    self.split(//).map do |letter|
      offset = (letter == letter.upcase ? 65 : 97)
      ((letter.ord-offset + shift) % 26 + offset).chr
    end.join
  end
end
...
class Hash
  def difference(other_hash)
    def find_uniques hash1, hash2
      hash1.select do |key, val|
        hash2[key].nil?
      end
    end
    
    find_uniques(self, other_hash).merge(
      find_uniques(other_hash, self))
  end
end
...
class Fixnum
  def stringify(base)
    ...
    (0..9).each do |num|
        hex_digits[num] = num.to_s
    end

    power = 0
    output = ""
    while self >= base ** power
        digit = hex_digits[(self / (base ** power)) % base]
        output = digit + output
        power += 1
    end
    output
