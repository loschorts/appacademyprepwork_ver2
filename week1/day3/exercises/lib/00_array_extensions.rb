  ...
  def two_sum
    ...
    (0...self.length).each do |first|
      (first+1...self.length).each do |second|
        output << [first, second] if self[first] + self[second] == 0
      end
    end
    output
  end
...
class Array
  def median
    ...
    new = self.sort
    if new.length % 2 == 0 
      (new[length/2].to_f + new[(length/2)-1]) / 2
    else
      new[new.length/2]
    end
  end
...
class Array
  ...
  def my_transpose
  	new = []
  	(0...self[0].length).each do |column|
	  	new_row = []
  	    (0...self.length).each do |index|
  	    new_row << self[index][column]
  		end
		new << new_row
	  end
	  new
