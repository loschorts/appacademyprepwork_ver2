...
class Array
  def bubble_sort! &block
    ...
    while not sorted
      sorted = true
      (0..new.length-2).each do |index|
        if block.call(new[index], new[index+1]) == 1
...
def substrings(string)
  ...
  (0...string.length).each do |start|
    (start...string.length).each do |finish|
      sub_strings << string[start..finish]
    end
  end
  sub_strings
end
...
class Array
  ...
  def my_inject(output = nil, &prc)
    new = self
    output = new.shift if output.nil?
    new.my_each do |el|
      output = prc.call(output, el)
    end
    output
  end
end
